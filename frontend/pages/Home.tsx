...
import { Filter, SlidersHorizontal, Heart, ShieldCheck, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  // Auto-scroll to products when search is active and data is loaded
  useEffect(() => {
    if (searchQuery && !isLoading && productsRef.current) {
      // Small delay to ensure smooth rendering before scroll
      setTimeout(() => {
        scrollToProducts();
      }, 100);
    }
  ...
  /* Manual filtering removed as backend handles it */
  ...
  return (
    <div className="pt-24 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8 pt-10 md:pt-20">
          <div className="animate-fade-in space-y-6">
            ...
            <h1 className="text-6xl md:text-9xl font-display font-extrabold text-white tracking-tighter leading-none pb-2">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-4">
              <button
                ...
                className="aura-btn px-8 py-4 rounded-2xl text-white font-bold text-sm uppercase tracking-widest flex items-center group transition-all"
              >
                Shop the Collection
          ...
          <div className="hidden lg:block relative animate-fade-in [animation-delay:300ms]">
            <div className="absolute inset-0 bg-primary/20 blur-3xl rounded-full opacity-30 animate-glow-pulse" />
            <div className="relative glass p-4 rounded-[3rem] border border-white/5 rotate-3 hover:rotate-0 transition-transform duration-700">
              <img
              ...
              />
              <div className="absolute -bottom-6 -left-6 glass p-6 rounded-3xl border border-white/10 shadow-2xl animate-slide-up">
        ...
        {/* Categories Bar */}
        <div ref={productsRef} className="flex items-center justify-between mb-8 animate-fade-in [animation-delay:400ms]">
          <div className="flex items-center space-x-3 overflow-x-auto pb-2 no-scrollbar scroll-smooth">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`px-6 md:px-8 py-2.5 md:py-3 rounded-full text-[10px] md:text-xs font-black whitespace-nowrap transition-all border ${selectedCategory === cat
          ...
          {/* {/* <button className="hidden md:flex items-center space-x-2 px-6 py-3 glass rounded-full text-xs font-bold text-zinc-400 hover:text-white transition-all hover:bg-zinc-800">
          ...
          <div className="animate-fade-in [animation-delay:600ms]">
            {featuredProducts.length > 0 ? (
                ...
                {totalPages > 1 && (
                  <div className="mt-12 flex items-center justify-center gap-4">
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm uppercase tracking-wider transition-all ${currentPage === 1
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm uppercase tracking-wider transition-all ${currentPage === totalPages
                        ? 'bg-zinc-900/50 text-zinc-700 cursor-not-allowed border border-white/5'
                        : 'aura-btn text-white hover:scale-105'
                        }`}
                    >
              ...
              <div className="py-32 text-center glass rounded-[3rem] border border-white/5">
                <Filter className="w-16 h-16 text-zinc-800 mx-auto mb-6" />
