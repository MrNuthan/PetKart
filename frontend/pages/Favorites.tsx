...
const Favorites: React.FC = () => {
    ...
    const handleAddToCart = async (product: any) => {
        ...
        // Keep product in favorites (don't auto-remove)
    ...
    const handleMoveAllToCart = async () => {
        ...
        // Optional: You can choose to clear all favorites after moving to cart
        // for (const product of favorites) {
    ...
    if (favorites.length === 0) {
        return (
            <div className="pt-24 pb-20 max-w-7xl mx-auto px-4 text-center animate-fade-in">
    ...
    return (
        <div className="pt-24 pb-20 overflow-x-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* Favorites Items List - Single Column Layout */}
                <div className="space-y-4 md:space-y-6 max-w-4xl">
                    {favorites.map((product) => (
                        <div
                            key={product.id}
                            className={`glass p-4 md:p-6 rounded-2xl md:rounded-3xl border border-white/5 flex flex-row items-center space-x-4 md:space-x-6 transition-all duration-300 ${removingItem === product.id
                        ...
                        >
                            ...
                            <div className="flex flex-col items-end space-y-2 md:space-y-3">
                                ...
                                <button
                                    ...
                                    className={`flex items-center px-3 md:px-4 py-2 md:py-2.5 bg-primary hover:bg-primary-hover text-white font-bold rounded-lg md:rounded-xl transition-all text-[10px] md:text-xs uppercase tracking-widest shadow-lg shadow-primary/20 ${addingToCart === product.id
